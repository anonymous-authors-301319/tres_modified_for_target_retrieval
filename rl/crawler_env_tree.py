...
class TreeCrawlerEnv(gym.Env):
    ...
    def step(self, action: int):
        ...
        try:
            ...
            self.state, new_url = self._take_action(action)     # state representation of the fetched page
            ...
            for message in self.crawler_sys.false_messages:
                try:
                    if re.search(message, self.state_body[:100]):
                        self.current_step -= 1
                        return new_url, False, None, False, {}
                except:
                    pass
            t4 = time.time()
        ...
        return new_url, self.state_webpage, reward, False, {}     # done = False
    ...
    def _take_action(self, action:int):
        ...
        t1 = time.time()
        new_url, self.state_body, self.state_title = self.crawler_sys.visit(url, content="both")     # text
        t2 = time.time()
    ...
    def extractStateActions(self, new_url):
        '''
        ...
        '''
        # Extract actions
        extracted = self.crawler_sys.expand(self.state_webpage, new_url) 
    ...
    def reset(self):
        ...
        for url in self.seed_urls:
            ...
            extracted = self.crawler_sys.expand(seed_webpage, new_url = None)
