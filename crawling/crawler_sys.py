...
class CrawlerSys(URLextractor, HTMLParser, TextReprGenerator):
    ...
    def visit(self, url, content="body"):
        ...
        # Only title
            ...
            if title == "" or title is None: 
                title = "Access denied, permission denied, not a given URL."
            return new_url, body.lower(), title.lower()  
        else:
            print("No param given in content")
            return None

    def expand(self, webpage, new_url, pooling="", padding=True, maxseqlen=MAXSEQLEN):
        '''
            A method for: 
        ...
        extracted_urls, extracted_titles = self.extractURLS(webpage.url, new_url, html=self.html)      
